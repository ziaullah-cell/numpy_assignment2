...
def function8():
    ...
    x = np.repeat(6,10).reshape(2,5).astype(dtype=np.uint32) #write your code here
...
def function12():
    ...
    arr = np.array([1,2,3])
    a = np.stack((arr,arr,arr), axis=-1) #write your code here 
    b = np.stack((arr,arr,arr), axis=0)
...
def function17():
    ...
    arr = np.where((arr%2==0) & (arr%5==0),"YES","NO")
    return arr          # Write Your Code HERE
...
def function18():
    ...
    x1 = np.intersect1d(piaic,students)
    x = np.count_nonzero(x1) #you code Here
    return x
